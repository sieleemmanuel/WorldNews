...
):PagingDataAdapter<Article, ArticlePagingAdapter.ArticleViewHolder>(DiffCallback) {
    private val TAG = ArticlePagingAdapter::class.simpleName

    inner class ArticleViewHolder(private val binding: PreviewArticleItemBinding):RecyclerView.ViewHolder(binding.root) {
        ...
        fun bind(article: Article){
            binding.apply {
                ...
                article.publishedAt
                tvEllapsedTime.text = when {
                    (elapsedSecs/(3600.times(24)))>=1 -> {
                        root.context.getString(R.string.days_format,(elapsedSecs/(3600.times(24))).toInt())
                    }
                    (elapsedSecs/3600)>=1 -> {
                ...
                Glide.with(binding.root.context).load(article.urlToImage).centerCrop().apply(options).into(imgArticlePreview)
                btnBookmark.apply {
                    setOnClickListener {
